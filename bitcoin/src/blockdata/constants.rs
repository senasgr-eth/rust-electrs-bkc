...
pub const TARGET_BLOCK_SPACING: u32 = 60; // Briskcoin value
/// How many blocks between diffchanges.
pub const DIFFCHANGE_INTERVAL: u32 = 2102400 / 60; // Briskcoin: halving interval / block spacing
/// How much time on average should occur between diffchanges.
pub const DIFFCHANGE_TIMESPAN: u32 = 60 * 60 * 24; // Briskcoin: 1 day in seconds

#[deprecated(since = "0.31.0", note = "Use Weight::MAX_BLOCK instead")]
/// The maximum allowed weight for a block, see BIP 141 (network rule).
pub const MAX_BLOCK_WEIGHT: u32 = 32_000_000; // Briskcoin value

#[deprecated(since = "0.31.0", note = "Use Weight::MIN_TRANSACTION instead")]
/// The minimum transaction weight for a valid serialized transaction.
pub const MIN_TRANSACTION_WEIGHT: u32 = WITNESS_SCALE_FACTOR as u32 * 60; // Briskcoin compatible

/// The factor that non-witness serialization data is multiplied by during weight calculation.
pub const WITNESS_SCALE_FACTOR: usize = 4; // Briskcoin value
/// The maximum allowed number of signature check operations in a block.
pub const MAX_BLOCK_SIGOPS_COST: i64 = 80_000; // Briskcoin value
/// Mainnet (bitcoin) pubkey address prefix.
pub const PUBKEY_ADDRESS_PREFIX_MAIN: u8 = 16; // Briskcoin mainnet
/// Mainnet (bitcoin) script address prefix.
pub const SCRIPT_ADDRESS_PREFIX_MAIN: u8 = 5; // Briskcoin mainnet
/// Test (signet, regtest) pubkey address prefix.
pub const PUBKEY_ADDRESS_PREFIX_TEST: u8 = 111; // Briskcoin testnet
/// Test (tesnet, signet, regtest) script address prefix.
pub const SCRIPT_ADDRESS_PREFIX_TEST: u8 = 196; // Briskcoin testnet
// Regtest (junkcoin) pubkey address prefix.
pub const PUBKEY_ADDRESS_PREFIX_REGTEST: u8 = 111; // Briskcoin regtest
/// The maximum allowed script size.
pub const MAX_SCRIPT_ELEMENT_SIZE: usize = 520; // Briskcoin (unchanged)
/// How may blocks between halvings.
pub const SUBSIDY_HALVING_INTERVAL: u32 = 2_102_400; // Briskcoin value
/// Maximum allowed value for an integer in Script.
pub const MAX_SCRIPTNUM_VALUE: u32 = 0x80000000; // Briskcoin (unchanged)
/// Number of blocks needed for an output from a coinbase transaction to be spendable.
pub const COINBASE_MATURITY: u32 = 21; // Briskcoin value

/// Constructs and returns the coinbase (and only) transaction of the Bitcoin genesis block.
fn bitcoin_genesis_tx() -> Transaction {
    // Briskcoin genesis coinbase tx
    ...
    let in_script = script::Builder::new()
        ...
        .push_slice(b"The world is so beautiful")
        .into_script();
    ret.input.push(TxIn {
    ...
    // Outputs
    let script_bytes = hex!("0482f7a5b9c91a8635f3c6247d9278864411151a3a4b403cacddc62f2e1bad1e349c09e5c6679364f2409d9f83b996c37f92453ec3bfd283afb457cc8957f047d1");
    let out_script = script::Builder::new().push_slice(script_bytes).push_opcode(OP_CHECKSIG).into_script();
    ...
    match network {
        Network::Bitcoin => Block {
            header: block::Header {
                ...
                merkle_root,
                time: 1731634708, // Briskcoin mainnet
                bits: CompactTarget::from_consensus(0x1e0ffff0),
                nonce: 1052891, // Briskcoin mainnet
                aux_data: None,
            },
            txdata,
        },
...
impl ChainHash {
    ...
    pub const BITCOIN: Self = Self([0x30, 0xef, 0x8e, 0x08, 0x9d, 0xff, 0x57, 0x25, 0xc2, 0xf5, 0xef, 0x62, 0x68, 0x69, 0x54, 0x07, 0x36, 0x29, 0x67, 0x12, 0xaf, 0x5e, 0xe6, 0x29, 0xef, 0x77, 0x40, 0x44, 0x85, 0x8e, 0xe8, 0x00]);
    /// `ChainHash` for testnet bitcoin.
    pub const TESTNET: Self = Self([0x30, 0xef, 0x8e, 0x08, 0x9d, 0xff, 0x57, 0x25, 0xc2, 0xf5, 0xef, 0x62, 0x68, 0x69, 0x54, 0x07, 0x36, 0x29, 0x67, 0x12, 0xaf, 0x5e, 0xe6, 0x29, 0xef, 0x77, 0x40, 0x44, 0x85, 0x8e, 0xe8, 0x00]);
    /// `ChainHash` for signet bitcoin.
    pub const SIGNET: Self = Self([0x30, 0xef, 0x8e, 0x08, 0x9d, 0xff, 0x57, 0x25, 0xc2, 0xf5, 0xef, 0x62, 0x68, 0x69, 0x54, 0x07, 0x36, 0x29, 0x67, 0x12, 0xaf, 0x5e, 0xe6, 0x29, 0xef, 0x77, 0x40, 0x44, 0x85, 0x8e, 0xe8, 0x00]);
    /// `ChainHash` for regtest bitcoin.
    pub const REGTEST: Self = Self([0x30, 0xef, 0x8e, 0x08, 0x9d, 0xff, 0x57, 0x25, 0xc2, 0xf5, 0xef, 0x62, 0x68, 0x69, 0x54, 0x07, 0x36, 0x29, 0x67, 0x12, 0xaf, 0x5e, 0xe6, 0x29, 0xef, 0x77, 0x40, 0x44, 0x85, 0x8e, 0xe8, 0x00]);
